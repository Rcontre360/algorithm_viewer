...
describe("Graph reducer should return proper state", () => {
  test("Add edge", () => {
    ...
    const response = graphReducer(graphState, {
      type: actions.ADD_EDGE,
      payload: [...graphData],
    });
    expect(response.data).toStrictEqual(graphData);
  });
  ...
  test("Set directed", () => {
    const response1 = graphReducer(graphState, {
      type: actions.SET_DIRECTED,
      payload: true,
    });
    const response2 = graphReducer(graphState, {
      type: actions.SET_DIRECTED,
      payload: false,
    });
