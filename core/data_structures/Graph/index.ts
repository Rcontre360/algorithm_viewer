...
export class Graph<T extends GraphType> implements GraphInterface<T> {
  ...
  setGraphOptions = (newOptions: IGraphOptions<T> | Function) => {
    if (newOptions instanceof Function) this.options = newOptions(this.options);
  ...
  connectNodes = (nodeSource: number | T, nodeDest: number | T): void => {
    ...
    if (!this.directed) this.nodes[nodeDest].push(nodeSource);
  };

  setEdgeWeight = (edge: number, weight: number) => {
    this.edges[edge].weight = weight;
  };

  deleteNode = (nodeDeleted: number): void => {
    if (this.size <= nodeDeleted) return;
    const filterFunction = (
      nodeConnections: number[] | T | number,
      index: number
    ) => index !== nodeDeleted;
  ...
  private getNodeNumber = (object: T | number): number => {
    if (typeof object === "number") return object as number;
  ...
  private applyOptions = (type: GraphOptionsKeys, ...args: T[]) => {
    if (this.options[type] instanceof Function) this.options[type](...args);
  };
}
