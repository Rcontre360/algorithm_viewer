...
export class GraphCase<T extends GraphType> {
  ...
  set canAddEdge(value: boolean) {
    if (value === true) this._canAddNode = false;
    this._canAddEdge = value;
  }

  set canAddNode(value: boolean) {
    if (value === true) this._canAddEdge = false;
  ...
  addEdge = (src: number | T, dest: number | T) => {
    if (this._canAddEdge) {
      if (this.onAddEdge) this.onAddEdge();
  ...
  setEdgeWeight = (edgeIndex: number, weight: number) => {
    if (edgeIndex < 0 || edgeIndex >= this.graph.getEdges().length)
      throw new Error("you cant assign weight to a nonexistent edge");
    this.graph.setEdgeWeight(edgeIndex, weight);
  };
  ...
  startAlgorithm = (algorithm: StartAlgorithm, options?: object) => {
    ...
    const returnVal = algorithmData.map((obj) =>
      this.parseReturnValue(obj, edges)
    );
    return returnVal;
  };
  ...
  private parseReturnValue = (obj: GraphReturn, edges: IGraphEdge[]) => {
    ...
    if (obj.from >= 0) newObject.fromData = this.graph.getNodeData(obj.from);

    if (obj.to >= 0) newObject.toData = this.graph.getNodeData(obj.to);

    if (obj.from >= 0 && obj.to >= 0)
      newObject.edgeIndex = edges.findIndex((edge) =>
        this.isRightEdge({ ...edge }, obj)
      );
  ...
  private isRightEdge = (edge: IGraphEdge, node: GraphReturn) => {
    return (
      (edge.nodeSrc === node.from && edge.nodeDest === node.to) ||
      (edge.nodeSrc === node.to && edge.nodeDest === node.from)
    );
  };
}
